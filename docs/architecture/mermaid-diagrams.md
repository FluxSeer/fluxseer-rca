...
- current release identity: `v0.3.0-beta.3`
...
Important `v0.3.0-beta.3` boundary: direct `RiskRule` RCA receives gateway-level hosted-provider egress opt-in enforcement, but the canonical `InvestigationRequest` path remains the only path with full `status.execution.egressAudit` visibility.
...
sequenceDiagram
    ...
    alt datasource or capability issue
        ...
        Note over RR,GW: v0.3.0-beta.3 direct RiskRule RCA applies gateway-level hosted-provider egress opt-in, but does not own canonical egressAudit status.
...
This diagram describes the canonical `InvestigationRequest` path in `v0.3.0-beta.3`.
...
The `local-filesystem` normalized snapshot store is development-oriented in `v0.3.0-beta.3`: it is not encrypted, has no automatic snapshot garbage collection, and should not be treated as durable production evidence storage.
...
This diagram records the `v0.3.0-beta.3` release path used by the project. The `release.yml` workflow is triggered by the annotated tag; the `test -> main -> tag` path is release discipline, not a hard workflow dependency.
...
flowchart LR
    ...
    MainCI[main CI]
    Tag[annotated tag v0.3.0-beta.3]
