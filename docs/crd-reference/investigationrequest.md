...
`status.execution.providerResult` is the durable normalized provider checkpoint. It stores the common RCA result, schema version, provider request ID when available, and digest used by FluxAgent after provider response parsing and validation. It does not store raw provider responses, prompts, chain-of-thought, or unclassified provider metadata.
