...
for rule in "${rules[@]}"; do
  "${cli_bin}" report riskrule "${rule}" -n "${control_namespace}" -o json >"${report_dir}/incidents/${rule}.json"
  bash "${repo_root}/hack/verify-riskrule-report.sh" "${report_dir}/incidents/${rule}.json"
done
